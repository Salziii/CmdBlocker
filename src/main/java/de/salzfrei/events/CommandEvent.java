...
public class CommandEvent implements Listener {
    ...
    public void onChat(ChatEvent e) {
        if (e.isCommand() && e.getSender() instanceof ProxiedPlayer) {
            ...
            if (Main.getType().isAllowed(u, "help") && Pattern.compile("help").matcher(e.getMessage()).find()) {
                ...
                sb.delete(sb.length()-1, sb.length()); // Letztes '/n' weg
            ...
            if (!Main.getType().isAllowed(u, command) || !validCommands.contains(command)) {
                e.setCancelled(true);
